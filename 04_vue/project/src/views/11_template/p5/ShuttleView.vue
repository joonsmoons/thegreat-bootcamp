  ...
  methods: {
    ...
    doAdd() {
      if (this.checkedItems.length > 0) {
        const addCustomer = this.customers.filter(
          (customer) => this.checkedItems.indexOf(customer.id) > -1
        )
        const addCustomerNew = addCustomer.filter(
          (customer) => this.customers2.indexOf(customer) === -1
        )
        this.customers2.push(...addCustomerNew)
        this.checkedItems = []
        // const existCustomer = [...this.customer2]
      }
    },
    doRemove() {
      if (this.checkedItems2.length > 0) {
        this.customers2 = this.customers2.filter((customer) =>
          this.checkedItems2.indexOf(customer.id)
        )
      }
    }
  }
